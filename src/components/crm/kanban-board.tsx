...
// Provides help text for each Kanban stage.
...
// Defines the left border color for cards in each stage.
...
// Maps task priority to a background color.
...
function KanbanColumnHeader({ title, count, onSort, isSorted }: { title: KanbanStage; count: number; onSort: () => void; isSorted: boolean; }) {
    return (
        <div className="flex items-center justify-between p-3 mb-2 sticky top-0 bg-secondary z-10">
            <TooltipProvider>
            ...
            {/* Sort button is not shown for terminal stages. */}
            {title !== 'Enrolled' && title !== 'Dropped' && (
                 <TooltipProvider>
                    <Tooltip delayDuration={0}>
                        <TooltipTrigger asChild>
                           <Button variant={isSorted ? "default" : "ghost"} size="icon" className="h-7 w-7" onClick={onSort}>
...
function KanbanColumn({ title, leads, onViewProfile }: { title: KanbanStage; leads: Lead[]; onViewProfile: (lead: Lead) => void; }) {
  ...
  return (
    <div className="flex flex-col w-80 shrink-0 h-full">
      ...
      <div className="flex-1 overflow-y-auto rounded-md bg-secondary p-2">
...
function KanbanCard({ lead, onClick }: { lead: Lead, onClick: () => void }) {
    ...
    return (
        <Card className={cn("hover:shadow-lg transition-shadow cursor-pointer border-l-4", stageBorderColors[lead.stage])} onClick={onClick}>
